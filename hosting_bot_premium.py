...
# ADMIN_ID = OWNER_ID # Removed for privacy
        ...
        if user is None or user.id != OWNER_ID:
            try:
                if update.message is not None:
                    bot.send_message(update.message.chat.id, "🔒 This bot is private. Access denied.")
                elif update.callback_query is not None:
                    bot.answer_callback_query(update.callback_query.id, "🔒 This bot is private. Access denied.", show_alert=True)
            except Exception:
                pass
            return CancelUpdate()
        return None
...
user_files = {}
# active_users = set() # Removed for privacy
user_subscriptions = {}
# admin_ids = {OWNER_ID} # Removed for privacy
bot_locked = True # Bot is always locked to owner
...
def load_data():
    ...
    try:
        ...
        logger.info(f"✅ 𝐃𝐚𝐭𝐚 𝐥𝐨𝐚𝐝𝐞𝐝: 0 𝐮𝐬𝐞𝐫𝐬 (𝐩𝐫𝐢𝐯𝐚𝐭𝐞 𝐦𝐨𝐝𝐞)")
...
""")
    ...
    try:
        ...
        failed += 1
        logger.error(f"❌ 𝐅𝐚𝐢𝐥𝐞𝐝 𝐭𝐨 𝐬𝐞𝐧𝐝 𝐧𝐨𝐭𝐢𝐟𝐢𝐜𝐚𝐭𝐢𝐨𝐧 𝐭𝐨 {OWNER_ID}: {e}")
...
def send_welcome(message):
    ...
    # add_active_user(user_id, message.from_user.username) # Removed for privacy
...
def _callback_analytics(call, user_id):
    ...
    total_users = 1 # Only owner is active
...
if __name__ == '__main__':
    ...
    logger.info(f"👥 𝐀𝐜𝐭𝐢𝐯𝐞 𝐔𝐬𝐞𝐫𝐬: 1 (𝐎𝐰𝐧𝐞𝐫 𝐎𝐧𝐥𝐲)")
